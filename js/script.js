...
FB.getLoginStatus(function(response) {
 ...
 //      document.getElementsByTagName('body').innerHTML = ""
    ...
    FB.api('/me/picture?type=large', function(response) { // normal/large/squere 
      var str="<img id=getPicUrl src="+ response.data.url +">";
      $("#preview1").append(str);
    });
...
//以下為canvas的程式碼，基本上不需多動，依據comments修改即可
  ...
  img.src = "img/overlay.png"; //圖像路徑（路徑自己設，且自己加入想要的圖層）
	...
	img2.src = "img/overlay3.jpg" //圖像路徑
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      ctx.clearRect(0,0,canvasWidth,canvasHeight); //移除canvas起始的內容
			var profileIMG = document.getElementById("getPicUrl");//抓html裡預載入的照片
      ...
      ctx.drawImage(img2,0,0); //劃入img2			
			...
			ctx.fillText(inputedText,241,466); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
      }
    }
