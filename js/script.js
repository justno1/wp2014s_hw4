  ...
  img.src = "img/overlay.png"; //圖像路徑（路徑自己設，且自己加入想要的圖層）
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      ctx.clearRect(0,0,canvasWidth,canvasHeight); //移除canvas起始的內容
			...
			ctx.drawImage(img3,canMouseX-1/2,canMouseY-30/2);//從XY軸0，0值開始畫如profileimg
			...
			ctx.fillText(inputedText, canMouseX-185,canMouseY-425); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
      }
    }
